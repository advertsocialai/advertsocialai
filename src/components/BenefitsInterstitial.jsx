  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="benefits-interstitial-grid-container w-full max-w-6xl">
          {/* Mobile */}
          ...
          <div className="hidden lg:block">
            {/* First Row */}
            <div className="benefits-interstitial-lg-grid grid grid-cols-3 gap-12 xl:gap-16">
              {items.slice(0, 3).map((item, index) => (
                <CardAnimated index={index} key={item.title}>
                  <div className="relative">
                    <BenefitCard item={item} />
                    {index < 2 && (
                      <div className="benefits-interstitial-vertical-divider absolute right-[-24px] xl:right-[-32px] top-0 w-px h-full bg-[#F5F5F7]" />
                    )}
                  </div>
                </CardAnimated>
              ))}
            ...
            {/* Horizontal Divider */}
            <div className="benefits-interstitial-divider w-full max-w-4xl h-px bg-[#F5F5F7] mx-auto my-12" />
...
const BenefitCard = ({ item }) => {
  return (
    <div className="benefits-interstitial-card flex flex-col items-start text-left gap-4 sm:gap-5 md:gap-6 p-6 sm:p-7 md:p-8 lg:p-0 bg-white rounded-[24px] sm:rounded-[20px] md:rounded-[18px] lg:rounded-none border border-gray-100 sm:border-gray-100 md:border-gray-100 lg:border-0 shadow-sm sm:shadow-sm md:shadow-sm lg:shadow-none">
      <img
        src={item.icon}
        alt=""
        className="benefits-interstitial-icon w-11 h-11 sm:w-12 sm:h-12 md:w-12 md:h-12 lg:w-12 lg:h-12"
      />
      <h3
        className="benefits-interstitial-card-title font-semibold text-lg sm:text-xl md:text-xl lg:text-lg text-gray-800 leading-tight"
        style={{ fontFamily: "Outfit" }}
      >
        {item.title}
      </h3>
      <p
        className="benefits-interstitial-card-desc text-gray-600 text-sm sm:text-base md:text-base lg:text-sm leading-relaxed"
        style={{ fontFamily: "Outfit" }}
      >
        {item.desc}
      </p>
