...
const BenefitCard = ({ item }) => {
  return (
    <div className="flex flex-col items-start text-left gap-4 sm:gap-5 md:gap-6 p-6 sm:p-7 md:p-8 lg:p-0 bg-white rounded-[24px] sm:rounded-[20px] md:rounded-[18px] lg:rounded-none border border-gray-100 sm:border-gray-100 md:border-gray-100 lg:border-0 shadow-sm sm:shadow-sm md:shadow-sm lg:shadow-none benefit-card-container">
      <img
        src={item.icon}
        alt=""
        className="w-11 h-11 sm:w-12 sm:h-12 md:w-12 md:h-12 lg:w-12 lg:h-12 benefit-card-icon"
      />
      <h3
        className="font-semibold text-lg sm:text-xl md:text-xl lg:text-lg text-gray-800 leading-tight benefit-card-title"
        style={{ fontFamily: "Outfit" }}
      >
        {item.title}
      </h3>
      <p
        className="text-gray-600 text-sm sm:text-base md:text-base lg:text-sm leading-relaxed benefit-card-desc"
        style={{ fontFamily: "Outfit" }}
      >
        {item.desc}
      </p>
